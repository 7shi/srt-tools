...
def fix_overlaps(srt):
    count = 0
    for i in range(len(srt.entries) - 1):
        if srt.entries[i].end > srt.entries[i + 1].start:
            srt.entries[i].end = srt.entries[i + 1].start
            count += 1
    return count
...
def setup_parser(subparsers):
    ...
    p.add_argument('--overlap', action='store_true', help='Only fix overlapping timestamps')
...
def do_command(args):
    ...
    for path in args.files:
        ...
        if do_overlap:
            count = fix_overlaps(srt)
            if count:
                changes.append(f"fixed {count} overlaps")
